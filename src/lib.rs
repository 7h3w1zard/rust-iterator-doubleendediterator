...
mod tests {
    ...
    fn inf() {
        ...
        #[test]
        fn deep() {
            assert_eq!(flatten(flatten(vec![vec![vec![0, 1, 2]]])).count(), 3);
        }
    
        #[test]
        fn deep_deep() {
            assert_eq!(flatten(flatten(flatten(vec![vec![vec![vec![0, 1, 2]]]]))).count(), 3);
        }
    
        #[test]
        fn ext_deep() {
            assert_eq!(vec![vec![vec![0, 1, 2]]].iter().our_flatten().our_flatten().count(), 3);
        }
}
